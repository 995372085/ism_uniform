...
def main(_argv):
    ...
    try:
        ...
        cap = cv2.VideoCapture("rtsp://admin:a1234567@10.34.142.34/cam/realmonitor?channel=1&subtype=0")
        # cap = cv2.VideoCapture("./test8.jpg")
        # cap = cv2.VideoCapture("rtsp://admin:a1234567@10.34.131.154/cam/realmonitor?channel=1&subtype=0")
        print("连接摄像头成功！")
    except:
        print("连接摄像头失败！")
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if map is not None:
                ...
                if not gj:
                    ...
                    exceptionFlag=True
                    saveFlag=True
                    cv2.putText(frame, "outoutout", (200, 200), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (255, 0, 0), 2)
        ...
        if saveFlag:
            cv2.imwrite(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))) + ".jpg", result)
        if FLAGS.output:
            out.write(result)
